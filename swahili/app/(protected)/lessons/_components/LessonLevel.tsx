...
import type { LessonLevel as LessonLevelType } from "@/lib/lesson-structure";

interface LessonLevelProps {
  level: LessonLevelType;
  levelIndex: number;
  xp: number;
  xpLoading: boolean;
  isLessonCompleted: (unitId: number) => boolean;
}

export function LessonLevel({ level, levelIndex, xp, xpLoading, isLessonCompleted }: LessonLevelProps) {
  const isLevelLocked = !xpLoading && level.requiredXp !== undefined && xp < level.requiredXp;

  return (
    <div className="animate-fade-in-up" style={{ animationDelay: `${levelIndex * 0.2}s` }}>
      {/* Level Header */}
